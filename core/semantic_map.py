...
class SemanticMap:
    ...
    def _init_faiss_index(self):
        ...
        try:
            if self.faiss_index_type.startswith("IDMap,"):
                ...
                logging.info(f"已创建 IndexIDMap 索引，基础类型: {base_index_type}")
            else:
    ...
    def _get_image_embedding(self, image_path: str) -> Optional[np.ndarray]:
        ...
        try:
            if self.image_model is None:
                ...
                return None
            emb = self.image_model.encode([image_path])
            # 取第一个结果
            if not isinstance(emb, np.ndarray):
                emb = np.asarray(emb)
            if emb.ndim > 1:
    ...
    def swap_out(
        ...
        """
        logging.info(
            f"swap_out: 开始换出，当前memory_units={len(self.memory_units)}，count={count}，strategy={strategy}"
        )
        if not self._external_storage:
        ...
        for uid, _ in units_to_page_out:
            ...
            # 同步到外部存储
            logging.info(
                f"swap_out: 正在同步单元 '{uid}' 到外部存储，所属空间: {space_names}"
            )
            if self._external_storage.add_unit(unit, space_names):
                synced_count += 1
                self._modified_units.discard(uid)
                logging.info(f"swap_out: 单元 '{uid}' 同步到外部存储成功")
            else:
                logging.error(f"swap_out: 单元 '{uid}' 同步到外部存储失败")
            ...
            if uid in self._uid_to_internal_faiss_id and self.faiss_index:
                try:
                    ...
                    logging.info(f"swap_out: 单元 '{uid}' 已从FAISS索引移除")
                except Exception as e:
            ...
            if uid in self.memory_units:
                ...
                removed_count += 1
                logging.info(f"swap_out: 单元 '{uid}' 已从内存移除")
        ...
        logging.info(
            f"swap_out: 使用{strategy}策略已将 {removed_count} 个单元从内存换出，其中 {synced_count} 个已同步到外部存储"
        )
    ...
    def add_unit(
        ...
        # 检查内存限制，必要时触发换页
        logging.info(
            f"add_unit: 当前memory_units={len(self.memory_units)}，最大={self._max_memory_units}"
        )
        if len(self.memory_units) >= self._max_memory_units:
            logging.info("add_unit: 触发swap_out")
            page_out_count = max(1, int(self._max_memory_units * 0.1))  # 至少换出1个
            self.swap_out(count=page_out_count)
            logging.info("add_unit: swap_out完成")
        ...
        self.memory_units[unit.uid] = unit
        logging.info(
            f"记忆单元 '{unit.uid}' 已添加/更新到 SemanticMap。当前memory_units={len(self.memory_units)}"
        )
    ...
    def _search_with_single_space_index(
        ...
        if not space or not hasattr(space, "_emb_index") or space._emb_index is None:
            ...
            return self._search_with_global_index(
                query_embedding_np, k, space.get_all_unit_uids(recursive=True) if space else set()
            )

        # 使用空间的局部索引搜索
        try:
            search_result = space._emb_index.search(query_embedding_np, k)
    ...
    def _search_with_brute_force(
        ...
        """使用暴力搜索（适用于多空间或指定候选单元），返回 (unit, 距离)"""
        ...
        try:
            ...
            search_result = index.search(query_embedding_np, min(k, len(candidate_units)))
            if search_result is None or len(search_result) != 2:
                return []
            distances, indices = search_result
            if distances is None or indices is None:
    ...
    def add_memory_space(self, space: "MemorySpace"):
        ...
        for uid in space.get_all_unit_uids():
            if uid not in self.memory_units:
                unit = space.get_unit(uid)
                if unit is not None:
                    self.memory_units[uid] = unit
