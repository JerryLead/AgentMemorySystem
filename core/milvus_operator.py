...
class MilvusOperator:
    ...
    def _ensure_collection(self) -> bool:
        ...
        try:
            ...
            try:
                ...
                logging.debug(f"集合加载时出现异常（可能已加载）: {e}")
    ...
    def _create_collection(self) -> bool:
        ...
        try:
            ...
            fields = [
                # 主键字段
                FieldSchema(
                    name="uid", dtype=DataType.VARCHAR, is_primary=True, max_length=256
                ),
                # 记忆单元基本信息
                ...
                FieldSchema(name="access_count", dtype=DataType.INT64, default_value=0),
                FieldSchema(
                    name="content_type",
    ...
    def _create_scalar_indexes(self):
        ...
        for field_name, params in scalar_indexes:
            try:
                self.collection.create_index(
                    field_name=field_name, index_params={"index_type": "Trie"}
                )
    ...
    def get_units_batch(self, unit_ids: List[str]) -> List[MemoryUnit]:
        ...
        try:
            ...
            results = self.collection.query(
                expr=expr, output_fields=["uid", "raw_data", "metadata", "embedding"]
            )
    ...
    def unit_exists(self, unit_id: str) -> bool:
        ...
        try:
            expr = f'uid == "{unit_id}"'
            results = self.collection.query(expr=expr, output_fields=["uid"], limit=1)
    ...
    def update_unit_metadata(
        self, unit_id: str, metadata: Dict[str, Any], merge: bool = True
    ) -> bool:
        """
        更新记忆单元的元数据
    ...
    def search_similarity_by_text(
        self, query_text: str, k: int = 5, **kwargs
    ) -> List[Tuple[MemoryUnit, float]]:
        """
    ...
    def search_in_memory_spaces(
        ...
        return self.search_similarity_by_vector(
            query_embedding=query_embedding, k=k, space_names=space_names
        )
    ...
    def get_unit_spaces(self, unit_id: str) -> Optional[List[str]]:
        ...
        try:
            expr = f'uid == "{unit_id}"'
            results = self.collection.query(
                expr=expr, output_fields=["memory_spaces"], limit=1
            )
    ...
    def get_units_in_spaces(
        self, space_names: List[str], recursive: bool = True
    ) -> List[MemoryUnit]:
        """
        获取指定空间中的所有记忆单元
        ...
        try:
            ...
            for space_name in space_names:
                space_conditions.append(
                    f'array_contains(memory_spaces, "{space_name}")'
                )
    ...
    def count_units(
        ...
        try:
            # 构建过滤条件
            filter_expr = self._build_filter_expression(
                space_names=space_names, content_types=content_types
            )

            if filter_expr:
                results = self.collection.query(expr=filter_expr, output_fields=["uid"])
                return len(results)
            else:
    ...
    def _build_filter_expression(
        ...
        if space_names:
            space_conditions = [
                f'array_contains(memory_spaces, "{space}")' for space in space_names
            ]
    ...
    def _parse_search_hit(self, hit) -> Optional[MemoryUnit]:
        ...
        try:
            ...
            return MemoryUnit(
                uid=unit_id, raw_data=raw_data, metadata=metadata, embedding=embedding
            )
...
if __name__ == "__main__":
    ...
    with MilvusOperator(
        ...
        for i, unit in enumerate(test_units):
            ...
            success = milvus_op.add_unit(unit, space_names=spaces, content_type="text")
        ...
        search_results = milvus_op.search_similarity_by_vector(
            query_embedding=query_vector, k=3, space_names=["space_0", "global_space"]
        )
