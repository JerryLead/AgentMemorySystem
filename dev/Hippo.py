...
class MemorySpace:
    ...
    def __init__(
        ...
        self.emb_index = faiss.IndexFlatL2(embedding_dim)
    ...
    def add_unit(self, unit: MemoryUnit):
        ...
        if not isinstance(unit, MemoryUnit):
            raise TypeError(f"尝试向记忆空间{self.name}添加无效单元{unit}")
        if unit in self._units:
            raise ValueError(f"尝试向记忆空间{self.name}添加重复单元{unit}")
    ...
    def remove_unit(self, uid: str):
        ...
        if uid not in self._units:
            raise ValueError(f"尝试从记忆空间{self.name}中移除不存在的记忆单元{uid}")
    ...
    def update_unit(self, unit: MemoryUnit):
        if unit.uid not in self._units:
            raise ValueError(f"尝试从记忆空间{self.name}更新不存在的记忆单元{unit}")
    ...
    def get_unit(self, uid: str):
        ...
        if not unit:
            raise ValueError(f"尝试从记忆空间{self.name}中获取不存在的记忆单元{uid}")
        return unit
    ...
    def build_index(self):
        self.emb_index = faiss.IndexFlatL2(self.emb_index.shape[1])
        ...
        for unit in self.get_all_units():
            if unit.embedding:
                ...
                self.emb_index.add(unit.embedding)
                count += 1
...
class SemanticMap:
    ...
    def __init__(
        ...
        self.emb_index = faiss.IndexFlatL2(embedding_dim)  # 全局索引
        self.index_to_unit: Dict[int, MemoryUnit] = {}

        self._memory_spaces: Dict[str, MemorySpace] = (
            {}
        )  # 存储 MemorySpace 对象，键为 ms_name
        self.reverse_index: Dict[str, List[str]] = {}  # uid -> namems_name 快速查找
        ...
        if test_text_emb.shape[0] != embedding_dim:
            raise ValueError(
                f"文本模型输出维度 {test_text_emb.shape[0]} 与指定的 embedding_dim {embedding_dim} 不符。"
            )
    ...
    def add_unit(
        ...
        if text and img:
            raise ValueError(f"尝试添加的记忆单元{unit}有误，不能同时包含文本和图片")
        if text:
        ...
        for ms_name in ms_names:
            ...
            if not ms:
                raise ValueError(f"记忆空间{ms_name}不存在")
            ms.add_unit(unit)

            if not self.reverse_index.get(unit.uid):
                self.reverse_index[unit.uid] = []
            self.reverse_index[unit.uid].append(ms_name)
    ...
    def remove_unit(self, uid: str, rebuild_index_immediately: bool = False):
        """从语义地图中删除一个记忆单元。"""
        if uid not in self.reverse_index:
            raise ValueError(f"UID {uid}不存在")

        ms_names = self.reverse_index[uid]
        for ms_name in ms_names:
            self._memory_spaces[ms_name].remove_unit(uid)
        self.reverse_index.pop(uid)
    ...
    @property
    def update_unit(self, unit: MemoryUnit):
        if unit.uid not in self.reverse_index:
            raise ValueError(f"UID {unit.uid}不存在")

        ms_names = self.reverse_index[unit.uid]
        for ms_name in ms_names:
    ...
    def get_unit(self, uid: str) -> Optional[MemoryUnit]:
        ...
        ms_name = self.reverse_index[uid][0]
    ...
    def build_index(self):
        ...
        """
        self.emb_index = faiss.IndexFlatL2(self.embedding_dim)
        self.index_to_unit = {}
        count = 0
    ...
    def search_similarity_units_by_vector(
        ...
        返回:
            List[Tuple[MemoryUnit, float]]: 一个元组列表，每个元组包含 (相似的MemoryUnit, 相似度得分/距离)。
                                            列表按相似度降序排列 (距离越小越相似)。
        """
        if not self.emb_index or self.emb_index.ntotal == 0:
            raise ValueError("向量索引未初始化或为空")
        ...
        results = []
        D, I = self.emb_index.search(query_vector_np, top_k)
        for i in range(len(I[0])):
            if I[0][i] == -1:
                continue
            unit = self.index_to_unit[I[0][i]]
    ...
    def create_memory_space(self, ms_name: str):
        ...
        if ms_name in self._memory_spaces:
            raise ValueError(f"记忆空间{ms_name}已存在")
...
class SemanticGraph:
    ...
    def add_unit(
        ...
        if self.nx_graph.has_node(unit.uid):
            raise ValueError(f"节点{unit.uid}已存在于SemanticGraph中")
    ...
    def add_explicit_edge(
        ...
        if not self.semantic_map.get_unit(src_uid):
            raise ValueError(f"源节点{src_uid}不存在")
        if not self.semantic_map.get_unit(tgt_uid):
            raise ValueError(f"目标节点{tgt_uid}不存在")
    ...
    def delete_memory_unit(
        self, unit_id: str, rebuild_semantic_map_index_immediately: bool = False
    ):
        """从图谱和底层的 SemanticMap 中删除一个记忆单元及其所有相关关系。"""
        # 1. 从 SemanticMap 删除
        self.semantic_map.remove_unit(
            unit_id, rebuild_index_immediately=rebuild_semantic_map_index_immediately
        )
    ...
    def search_similarity_in_graph(
        ...
        if (
            ...
            raise ValueError(
                "query_text,query_vector, query_image_path只能有一个不为None"
            )
    ...
    def get_explicit_neighbors(
        ...
        if not self.nx_graph.has_node(uid):
            raise ValueError(f"记忆单元{uid}不在图中")
