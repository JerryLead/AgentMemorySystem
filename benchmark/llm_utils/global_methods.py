...
def run_chatgpt(query, num_gen=1, num_tokens_request=1000, 
                model='gpt-4o-mini', temperature=1.0, wait_time=1):
    """
    使用新版 openai.ChatCompletion 接口调用 OpenAI GPT-3.5/4 系列模型。
    支持多轮重试，自动处理常见API异常。
    """
    if os.environ.get("OPENAI_API_KEY") is None:
        raise ValueError("请设置环境变量 OPENAI_API_KEY 以使用 OpenAI API。")
    client = OpenAI(
    ...
    while retries < max_retries:
        try:
            ...
            retries += 1
            wait_time = min(wait_time * 2, 30)
    raise RuntimeError("OpenAI API调用失败，已重试多次。请检查API密钥和网络连接。")
...
def run_deepseek(query, num_gen=1, num_tokens_request=1000,
                model='deepseek-chat', temperature=1.0, wait_time=1):
    """
    使用deepseek的API调用模型。
    支持多轮重试，自动处理常见API异常。
    """
    if os.environ.get("DEEPSEEK_API_KEY") is None:
        raise ValueError("请设置环境变量 DEEPSEEK_API_KEY 以使用 DEEPSEEK API。")
    client = OpenAI(
...
if __name__ == "__main__":
    # 测试 run_chatgpt 和 run_deepseek 的功能

    # 请确保已设置 OPENAI_API_KEY 和 DEEPSEEK_API_KEY 环境变量
    print("测试 run_chatgpt ...")
    try:
        query = "请用一句话介绍一下人工智能。"
        answer = run_chatgpt(query, model="gpt-3.5-turbo", num_tokens_request=64)
        print("[run_chatgpt] 返回结果：", answer)
    except Exception as e:
        print("run_chatgpt 测试失败：", e)

    print("\n测试 run_deepseek ...")
    try:
        query = "请用一句话介绍一下人工智能。"
        answer = run_deepseek(query, model="deepseek-chat", num_tokens_request=64)
        print("[run_deepseek] 返回结果：", answer)
    except Exception as e:
        print("run_deepseek 测试失败：", e)
